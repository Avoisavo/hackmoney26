...
export default function YellowNetworkPage() {
        ...
        try {
            ...
            if ((switchError as { code: number }).code === 4902) {
                try {
                    ...
                    addMessage('error', 'Failed to add Sepolia network');
                }
            }
        }
    };
    ...
    const connectWallet = async () => {
        ...
        try {
            ...
            const isCorrectChain = chainId === SEPOLIA_CONFIG.chainId;
    ...
    useEffect(() => {
        ...
        const handleChainChanged = (...args: unknown[]) => {
            ...
            addMessage('info', `Chain changed to ${chainId === SEPOLIA_CONFIG.chainId ? 'Sepolia' : 'Unknown'}`);
        };
    ...
    const connectToClearNode = useCallback(() => {
        ...
        try {
            ...
            ws.onmessage = (event) => {
                try {
                    // Use standard JSON.parse as in index.ts for better reliability
                    ...
                    console.log('Raw or invalid message:', event.data, err);
        ...
        switch (msgType) {
            ...
            case 'bu': // Balance Update (Push from node)
                if (result && result.balance_updates) {
                    ...
                    addMessage('info', `💰 Balance Sync: ${bus.map((b: any) => `${b.amount} ${b.asset}`).join(', ')}`);
                }
                break;
            case 'channels': // List of channels (Push from node)
    ...
    const requestSessionAuth = async () => {
        ...
        try {
            ...
            addMessage('info', 'Generating local session key (viem style)...');

            // Generate local session key using viem (matches index.ts)
            if (!sessionPrivateKeyRef.current) {
                sessionPrivateKeyRef.current = generatePrivateKey();
            }
            
            const sessionAccount = privateKeyToAccount(sessionPrivateKeyRef.current);
            const sessionAddress = sessionAccount.address;

            // Use BigInt for expires_at (matches index.ts)
            ...
            console.log('🚀 Sending auth_request (SDK Builder):', authRequestMsg);
            wsRef.current.send(authRequestMsg);
            addMessage('sent', 'Auth request sent. Waiting for challenge...');
        } catch (err: any) {
    ...
    const verifySession = useCallback(async (challenge: string) => {
        const currentWallet = walletRef.current;
        console.log('🚀 verifySession triggered with challenge:', challenge);
        console.log('Current Wallet State in verifySession:', currentWallet);
        
        if (!window.ethereum || !pendingAuthRef.current || !sessionPrivateKeyRef.current || !currentWallet.address) {
            console.error('Missing state for verification:', {
                ethereum: !!window.ethereum,
                pendingAuth: !!pendingAuthRef.current,
                sessionKey: !!sessionPrivateKeyRef.current,
                address: currentWallet.address
            });
            addMessage('error', 'Wallet not connected or internal state missing.');
            return;
        }

        try {
            setAuth(prev => ({ ...prev, status: 'verifying' }));
            addMessage('info', 'ATTEMPTING TO TRIGGER METAMASK SIGNING...');

            // Create viem client explicitly for signing (ensures clean state)
            const wc = createWalletClient({
                chain: sepolia,
                transport: custom(window.ethereum as any),
            ...
            console.log('Creating EIP712 signer with address:', currentWallet.address);

            const signer = createEIP712AuthMessageSigner(
                wc as any,
                pendingAuthRef.current,
                { name: 'yellow-payment-demo' }
            );

            console.log('Calling createAuthVerifyMessageFromChallenge...');
            
            // This is where MetaMask SHOULD prompt
            const verifyMsg = await createAuthVerifyMessageFromChallenge(
                signer,
                challenge
            );

            console.log('✅ Signature obtained! Sending auth_verify:', verifyMsg);
            wsRef.current?.send(verifyMsg);
            addMessage('sent', 'Auth verification sent to ClearNode!');
        } catch (err: any) {
            console.error('❌ Verification failed/rejected:', err);
            addMessage('error', `Verification failed: ${err.message || err.toString()}`);
            setAuth(prev => ({ ...prev, status: 'unauthenticated' }));
        }
    }, [addMessage]);
    ...
    const createMessageSigner = useCallback(() => {
        ...
        // If we have an authenticated session key, use it for instant signing (SDK builder style)
        if (sessionPrivateKeyRef.current) {
            const sdkSigner = createECDSAMessageSigner(sessionPrivateKeyRef.current);
            return async (payload: unknown): Promise<`0x${string}`> => {
                // The SDK signer expects an object and returns a promise
                return await sdkSigner(payload as any);
            };
        }
    ...
    }, []); // walletRef and sessionPrivateKeyRef manage their own stale state
    ...
    // Create payment session (Channel + Resize/Fund)
    const createSession = async () => {
        ...
        const messageSigner = createMessageSigner();
        if (!messageSigner || !walletRef.current.address || !sessionPrivateKeyRef.current) {
            addMessage('error', 'Auth required. Please authenticate first.');
            return;
        }

        try {
            addMessage('sent', 'Initiating Channel Creation (create_channel)...');

            if (!sessionPrivateKeyRef.current) {
                addMessage('error', 'Session key missing. Re-authenticate.');
                return;
            }
            ...
            console.log('Sending create_channel (SDK Builder):', createChannelMsg);
            wsRef.current.send(createChannelMsg);
            addMessage('sent', 'Create channel request sent. Check logs for incoming "create_channel" message to sign...');
        } catch (err) {
            console.error('Failed to initiate flow:', err);
            addMessage('error', `Flow failed: ${err}`);
        }
    };
    ...
    const fundChannel = async (channelId: string) => {
        ...
        try {
            ...
            console.log('Sending resize_channel (SDK Builder):', resizeMsg);
            wsRef.current.send(resizeMsg);
            addMessage('sent', 'Resize (Funding) request sent. Wait for confirmation...');
        } catch (err) {
                ...
                <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-white/60">State Channel Demo on Sepolia</p>
                        </div>
                    </div>
            ...
            <main className="max-w-6xl mx-auto px-6 py-8">
                ...
                <div className="text-center mb-10">
                    ...
                    <p className="text-white/60 max-w-2xl mx-auto">
                        ...
                        through Yellow Network on Sepolia testnet.
                    </p>
                </div>
                ...
                <div className="grid lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-6">
                            ...
                            {!wallet.connected ? (
                                ...
                                <div className="p-4 bg-green-500/10 rounded-xl border border-green-500/30">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-semibold text-green-400">Connected to Sepolia</p>
                        ...
                        <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm text-white/60 mb-2">Amount (ytest.usd units)</label>
                                    <input
                                        type="text"
                                        value={paymentAmount}
...
// Connect to Yellow Network ClearNode (Sepolia)
const ws = new WebSocket('wss://clearnet-sandbox.yellow.com/ws');
...
ws.send(sessionMessage);`}</code>
            ...
            <footer className="border-t border-white/10 mt-12">
                <div className="max-w-6xl mx-auto px-6 py-6 text-center text-white/40 text-sm">
                    ...
                    <p className="mt-2">Network: Sepolia (Chain ID: 11155111)</p>
                </div>
            </footer>
        </div>
    );
