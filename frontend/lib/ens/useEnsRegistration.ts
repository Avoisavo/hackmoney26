...
// Public Resolver ABI for setAddr and setText functions
...
export const useEnsRegistration = ({
    ...
    const buildRegistrationParams = useCallback((secretValue: `0x${string}`): RegistrationParams | null => {
        ...
        // Encode the setText call for avatar if provided
        if (avatar) {
            data.push(encodeFunctionData({
                abi: PUBLIC_RESOLVER_ABI,
                functionName: 'setText',
                args: [node, 'avatar', avatar]
            }));
        }
    ...
    useEffect(() => {
        if (step === 'committing' && commitmentHash && !commitTxHash && !isCommitPending && !isCommitConfirming && !isCommitmentLoading) {
